...
# SPECIES
load("rTP_mcc.Rdata") # Dataset created in previous sections
glimpse(rTP_mcc)
...
library(tidyverse)
library(fishualize)
load("rTP_all_liz.Rdata") # made in previous section
...
# We want to prioritise species level results, so we will create and fill the column with this first
# record source
...
load("RLS_higher_sig.Rdata") # Made in previous section
...
# Combine our SIA values with the trait information (this forms another paper)
...
check = RLS_pathway %>%filter(trophic_guild == "Herbivore")
...
# Here we are assigning species listed as herbivores or excavators in the RLS data as "0"
...
# Fill in the "herbis" not covered by what we have sampled:
bottom = 
  RLS_pathway %>% 
  filter(is.na(BPCI_w_herbSIA))
...
load("RLS_pathway.Rdata") # Made in past section
...
# -----------------------------------------------------------
# NEW JUST RENAME DATA TO PROCEED FROM HERE
...
# LOAD DATA
load("RLS_trans_sampled.Rdata") # Made in previous sections
load("RLS_trans.Rdata") # Made in previous sections
